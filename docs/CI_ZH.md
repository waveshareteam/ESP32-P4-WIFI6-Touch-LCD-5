...
显示示例 07–12 为 LCD5 BSP 验证头 `d9a93c0` 和 HX8394 组件源码 `fc6e6d2` 使用临时精确
Git 固定版本（已在上游 PR #192 审查）。两者均会保持临时状态，直至各自发布到 registry。其默认配置
选择 revision-1.3/pre-v3 产品配置；仓库没有受维护的产品固件源码，因此不包含按 revision 区分的
产品固件任务。
