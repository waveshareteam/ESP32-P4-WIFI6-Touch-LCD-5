...
六个显示相关产品 BSP manifest 必须使用
`waveshare/esp_lcd_hx8394: '^1.0.3'`。这保留了随仓库提供且已经过产品审查的 v1.0.3
约定；registry v2 的 API 和面板初始化行为不同。只有在两条必需 ESP-IDF 版本线及目标硬件
都完成验证后，才能重新评估该约束；重新评估时也不得修改嵌入式上游 HX8394 内容。
